...
Since there are a lot of XML files in the databases, like this one, I need to learn about XML, which looks really daunting. [w3schools](https://www.w3schools.com/xml/xml_whatis.asp) is a good website for introduction to XML, yet its method of reading xml files is pretty complicated.
...
However, reading the xml files is pretty hard and I want to change it to data frame but I haven't found the right way yet.
```{r, eval = FALSE}
library(XML)
library(methods)
xml_tf <- xmlParse(tf)
